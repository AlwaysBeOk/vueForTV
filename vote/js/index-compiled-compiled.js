...
setContent.prototype = {
	commonCall: function commonCall() {
		this.shipei();
		this.interactiveLive('http://115.182.34.94/api/list/liveMessageList'); //互动直播
		this.liveList(1); //实况直播
		this.setliveChat(); //边看边聊
	...
	/* 实况直播 */
	liveList: function liveList(curPage) {
		var app = 'cbox',
	...
	liveListSuccess: function liveListSuccess(data) {
		var html = '';
		(function (self) {
			if (data.code == 0) {
				//连接成功
	...
	setOneItem: function setOneItem(item, ifTop) {
		var self = this;
	...
	//互动直播
	interactiveLive: function interactiveLive(url) {
		var _this = this;
		var _url = url + '?id=' + _itemid + '&t=jsonp&cb=setcontent.interactiveLiveSuccess&serviceId=cbox';
		this.getJsonp(_url);
	},

	interactiveLiveSuccess: function interactiveLiveSuccess(data) {
		var self = this;
		var data = data.data;
		//直播标题
		if (data) {
	...
	getVideoHeight: function getVideoHeight(width, ratio) {
		//计算视频高度
		var height;
		if (ratio == 0) {
			//4:3
	...
	// 边看边聊
	setliveChat: function setliveChat() {
		var app = 'cbox',
	...
	setliveChatSuccess: function setliveChatSuccess(data) {
		var html = '';
		var self = this;
		if (data.code == 0) {
	...
	getReplyContent: function getReplyContent(pid, data) {
		var _html = '';
	...
	// 多视角
	setMultipleList: function setMultipleList(data) {
		var html = '';
		var live = ' live';
		var self = this;
		if (data.remark == 0) {
   ...
   if($('.duoshijiao') && $('.duoshijiao').length > 0){
	...
	loadVideoList: function loadVideoList(data) {
		var html = '';
		var self = this;
		$('#videoList').html('');
		$.each(data.liveList, function (index, item) {
			...
			if (data.remark == 0) {
				...
				li_html = '<div class="swiper-slide" data-type=' + item.liveType + ' data-url=' + item.liveUrl + '>' + live_html + '<div class="n-navWord">' + item.liveTitle + '</div>' + '<img src="' + item.liveImage + '" alt="" />' + '</div>';
	...
	h5playerZhibo: function h5playerZhibo(channel_name, video_id, width, height) {
		/*直播播放器*/
		var attributes = {
	...
	h5playerDianbo: function h5playerDianbo(div_id, pid, width, height, posterImg) {
		/*点播播放器*/
	...
	h5player: function h5player(type, url, divid, width, height, posterImg) {
	...
	changeIndexVideo: function changeIndexVideo(type, url) {
		if ($('#' + self.indexVideoDiv).length > 0) {
			var height = parseInt($('html').css('fontSize')) * 9;
...
LiveFunction.prototype = {
	...
	xiuChangSuccess: function xiuChangSuccess(data) {
		//title
		...
		//author
		if (data.data.msgCount > 0 || data.msgList.length > 0 || data.data.countFlag == 0) {
			//20161215
			$('.live-status').show();
...
var bindApp = function bindApp() {
	this.clickClient();
};
bindApp.prototype = {
	clickClient: function clickClient() {
	...
	hasApp: function hasApp(url, fail) {
